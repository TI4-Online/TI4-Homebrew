...
const localeStrings = {
  ...
  "unit_modifier.desc.blight":
    "-1 to the results of non-blex players' combat rolls during the first round of combat in systems that contain Blight tokens",
  "unit_modifier.name.biotic_weapons": "Biotic Weapons",
  "unit_modifier.desc.biotic_weapons":
    "1 of your units may roll 1 additional combat die",
...
const factions = [
  {
    faction: "blex",
    abilities: ["contagion", "blight", "shared_misery"],
    commodities: 2,
    home: 3231,
    leaders: {
      agents: ["tox"],
...
const technologies = [
  ...
  },
  {
    ...
    cardNsid:
      "card.technology.unit_upgrade.blex:homebrew.discordant_stars/vector_2",
    type: "unitUpgrade",
...
const unitAttrs = [
  ...
  },
  {
    ...
    triggerNsid:
      "card.technology.unit_upgrade.blex:franken.discordant_stars/vector",
    move: 2,
  },
  {
    unit: "fighter",
...
const unitModifiers = [
  ...
  },
  {
    ...
    filter: (auxData) => {
      return ["groundCombat", "spaceCombat"].includes(auxData.rollType); // only affects combat
    },
    applyAll: (unitAttrsSet, auxData) => {
      ...
      for (const unitAttrs of unitAttrsSet.values()) {
        if (
          ...
          if (
            !best ||
            unitAttrs.raw[auxData.rollType].hit < best.raw[auxData.rollType].hit
          ) {
            best = unitAttrs;
          }
        }
