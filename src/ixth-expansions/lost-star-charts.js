...
if (
  world.TI4.ObjectNamespace.getNsid(refObject) !==
...
const spawnContent = (templateId, offset) => {
  ...
  const spawnedObject = world.createObjectFromTemplate(
    templateId,
    spawnPosition
  );
...
const spawnLloydComponents = () => {
  ...
  for (const templateId of Object.values(lloydTokens)) {
...
let spawnJarrahButton = new Button()
  ...
  .setFontSize(8);
spawnJarrahButton.onClicked.add(
  spawnContent.bind(this, jarrahTokenGuid, [8.5, 6, above])
);
...
let spawnKwonButton = new Button()
  ...
  .setFontSize(8);
spawnKwonButton.onClicked.add(
  spawnContent.bind(this, kwonTokenGuid, [8.5, 9, above])
);
